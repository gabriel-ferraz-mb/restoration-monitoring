...
for (name in files){
  ...
  # Calculate ndci
  NDCI <- (red_edge_1 - red) / (red_edge_1 + red)
  
  scl <- data[['SCL']]
  valid_mask <- scl == 6
  NDCI_masked <- mask(NDCI, mask = valid_mask, maskvalue = TRUE,inverse = TRUE)
  x.stats <- data.frame(talhao=talhao,
                        data=date,
                        ndci=cellStats(NDCI_masked, "mean"))
  
  result <- rbind(result,x.stats)
}

colnames(result) <- c("talhao", "date", "ndci_mean")

write.csv(result,
          "spreadsheets\\result_sites_ndci.csv",
          row.names = FALSE)
...
# colnames(tab)<-tab[1,]
# tab<-tab[2:nrow(tab),]
# tab$ndci_mean<-as.numeric(tab$ndci_mean)
...
for (label in labels){
  ...
  df <- data.frame(y_pred[(31):(length(y_pred) - 30)],
                   ...
                   l[(31):(length(l) - 30)])
  names(df) <- c('date','ndci_mean','talhao')
  #print(df)
  result_bspline <- rbind(result_bspline, df)
}
write.csv(result_bspline, "spreadsheets/bspline_ndci_lmite.csv", row.names=FALSE)

# result_bspline <- read.csv("spreadsheets/bspline_ndci_lmite.csv")
# result_bspline$ndci_mean<-as.numeric(result_bspline$ndci_mean)
...
for (i in labels){
  ...
  if (length(specific_date_index) > 0) {
    ...
    legend('bottomright', legend=c("Water bodie NDCI values"),
...
file.copy(from=plots.png.paths, to="png\\plots_ndci")
