...
write.csv(result,
          "spreadsheets\\result_sites_ndmi.csv",
          row.names = FALSE)
...
for (label in labels){
  ...
  df <- data.frame(y_pred[(31):(length(y_pred) - 30)],
                   ...
                   l[(31):(length(l) - 30)])
  names(df) <- c('date','ndmi_mean','talhao')
  #print(df)
  result_bspline <- rbind(result_bspline, df)
}
write.csv(result_bspline, "spreadsheets/bspline_ndmi_lmite.csv", row.names=FALSE)
...
file.copy(from=plots.png.paths, to="png\\plots_ndmi")
